...
use cim_keys::{
    pki::PkiManager, CertificateManager, CertificateMetadata, EcdsaCurve, KeyAlgorithm, KeyId,
    KeyManager, PkiOperations, Result, RsaKeySize,
};
...
fn test_certificate_templates() {
    ...
    fn get_template_config(
        ...
        match template {
            ...
            CertificateTemplate::CodeSigning => (vec!["digitalSignature"], vec!["codeSigning"]),
            CertificateTemplate::EmailProtection => (
                vec!["digitalSignature", "keyEncipherment"],
                vec!["emailProtection"],
            ),
            CertificateTemplate::ClientAuthentication => {
                (vec!["digitalSignature", "keyAgreement"], vec!["clientAuth"])
            }
