...
use cim_keys::{
    ...
    KeyId, KeyMetadata, KeyAlgorithm, KeyUsage, KeyLocation,
};
use chrono::Utc;
...
fn test_file_based_storage() {
    ...
    let key_filename = format!("{}.key", key_id);
    let key_path = storage_path.join(&key_filename);
    
    // Verify path construction
    assert!(key_filename.ends_with(".key"));
    assert_eq!(key_path.file_name().unwrap().to_str().unwrap(), key_filename);
    
    // Test metadata file
    let metadata_filename = format!("{}.json", key_id);
...
fn test_key_metadata_storage() {
    ...
    assert_eq!(extended.custom_attributes.get("project").unwrap(), "cim-keys");
