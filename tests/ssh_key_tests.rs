...
use cim_keys::{
    ssh::SshKeyManager,
    KeyManager, Signer,
    KeyId, KeyAlgorithm, KeyUsage, 
    SignatureFormat, KeyExportFormat,
...
impl MockSshKeyManager {
    ...
    fn generate_mock_key(&mut self, algorithm: KeyAlgorithm, label: String) -> KeyId {
        ...
        self.keys.insert(key_id, MockKey {
            algorithm,
            label,
            public_key,
            private_key,
...
fn test_ssh_key_algorithms() {
    ...
    let ed25519_id = manager.generate_mock_key(
        KeyAlgorithm::Ed25519,
...
fn test_ssh_key_export_formats() {
    ...
    let authorized_keys_line = format!("{} ssh-key-1", openssh_public);
