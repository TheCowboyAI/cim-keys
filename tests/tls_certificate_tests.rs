...
use cim_keys::{
    tls::TlsManager, CertificateFormat, CertificateManager, CertificateMetadata, EcdsaCurve,
    KeyAlgorithm, KeyExportFormat, KeyId, KeyManager, Result, RsaKeySize,
};
...
fn test_x509_distinguished_name() {
    ...
    // Build DN string
    let dn = format!("C={country}, ST={state}, L={locality}, O={organization}, OU={organizational_unit}, CN={common_name}, emailAddress={email}");
...
fn test_subject_alternative_names() {
    ...
    let ip_addresses = vec!["192.168.1.1", "10.0.0.1", "::1", "2001:db8::1"];

    let email_addresses = vec!["admin@example.com", "webmaster@example.com"];

    let uris = vec!["https://example.com", "ldap://ldap.example.com"];
...
fn test_certificate_revocation() {
    ...
    let ocsp_responders = vec!["http://ocsp.example.com", "http://ocsp2.example.com"];
