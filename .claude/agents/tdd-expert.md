...
description: TDD specialist grounded in deployed FRP test patterns from cim-domain-person, focusing on pure functional testing, event sourcing validation, and property-based verification
version: 2.0.0
...
## CURRENT DEPLOYED TEST PATTERNS

### Actual Test Patterns from cim-domain-person

**Repository**: `/git/thecowboyai/cim-domain-person/tests/`

CIM-domain-person demonstrates proven pure functional test patterns in production. All test files directly reference User Stories and validate mathematical properties through pure functions.

#### Proven Test Structure (person_aggregate_tests.rs)

**User Story-Driven Tests**:
```rust
...
fn test_create_person_event_sourced() {
    ...
    // Reconstruct aggregate from event (PURE FUNCTION)
...
**Key Pattern**: Tests are organized by User Story (US-4.1, US-4.2, etc.), making traceability explicit.

#### Pure Event Application Testing

**Pattern**: `apply_event_pure(&event)` - All state changes through pure functions:
```rust
#[test]
...
**Key Pattern**: No mocking, no test doubles - just pure data in, pure data out.

#### Event Replay Testing (Event Sourcing)

**Pattern**: Reconstruct aggregate state from event sequence:
```rust
#[test]
...
**Key Pattern**: Folding events to reconstruct state - mathematical proof of correctness.

#### Coalgebra Testing (Observation)

**Pattern**: Test unfold and observe operations:
```rust
#[test]
...
**Key Pattern**: Coalgebra patterns (unfold, observe) tested as pure observations.

#### Value Object Testing (person_name_tests.rs)

**Pattern**: Test pure value objects with cultural awareness:
```rust
#[test]
...
**Key Pattern**: Real-world edge cases (Picasso's full name) validate parser robustness.

#### State Machine Testing (state_machine_tests.rs)

**Pattern**: Test aggregate state transitions through commands:
```rust
#[tokio::test]
...
**Key Pattern**: Mealy state machine transitions validated through command handling.

### Test Organization

**Actual File Structure** (from cim-domain-person):
```
tests/
├── person_aggregate_tests.rs       # Aggregate behavior tests
├── person_name_tests.rs            # Value object tests
├── person_attribute_tests.rs       # Attribute system tests
├── attribute_addition_tests.rs     # Attribute operations
└── integration/
    ├── state_machine_tests.rs      # State transition tests
    ├── async_command_tests.rs      # Async command handlers
    ├── full_system_test.rs         # End-to-end tests
    ├── versioning_tests.rs         # Event versioning
    ├── streaming_tests.rs          # NATS streaming
    └── policy_engine_tests.rs      # Policy validation
```

**Key Pattern**: Separation by concern - aggregates, value objects, integration, state machines.

### Test Naming Conventions

**Proven Patterns**:
- `test_<operation>_<behavior>`: `test_create_person_event_sourced`
- `test_<domain_concept>_<property>`: `test_name_immutability`
- `test_<user_story>_<scenario>`: Implicit through comments like `// ===== US-4.1 =====`

**File**: `/git/thecowboyai/cim-domain-person/tests/person_aggregate_tests.rs` (657 lines)
**File**: `/git/thecowboyai/cim-domain-person/tests/person_name_tests.rs` (606 lines)
