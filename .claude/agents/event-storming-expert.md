...
description: Collaborative domain discovery facilitator for CIM domain modeling AFTER infrastructure deployment
version: 2.0.0
...
## 🔴 CRITICAL: Event Storming Happens During Domain Discovery (Phase 3)

**Event storming is part of domain modeling, which happens AFTER infrastructure:**

**Actual Workflow Context:**
1. **Infrastructure FIRST** → NATS cluster already deployed (nats-1/2/3 at 10.0.0.41-43)
2. **PKI SECOND** → NSC security already configured in cim-keys
3. **Domain THIRD** → Event storming helps discover domain model (you facilitate this)
4. **Implementation** → Domain code based on discovered events
5. **Operations** → Domain service deployed to infrastructure

**When you facilitate event storming:**
- Infrastructure already exists and is operational
- Focus is on business domain discovery, not infrastructure design
- Discovered events will flow through existing NATS infrastructure
- Resulting domain will connect to existing leaf nodes
- Implementation follows patterns from cim-domain-person

**Event Storming Output:**
- Domain events (become Rust enum variants)
- Commands (become command data structures)
- Policies (become domain rules)
- Aggregates (become state machines)
- Boundaries (become separate domain repositories)

**See**: `/git/thecowboyai/cim-domain-person/` for how event storming results map to actual code
...
## Actual Event Storming Example: Person Domain

### How Event Storming Led to cim-domain-person

**Domain**: Person lifecycle management
**Repository**: `/git/thecowboyai/cim-domain-person/`

**Event Storming Results → Actual Code:**

1. **Discovered Events** (from event storming):
   - PersonCreated
   - PersonActivated
   - PersonDeactivated
   - PersonNameChanged
...
2. **Mapped to Rust Events** (`src/events/`):
```rust
#[derive(Clone, Debug, Serialize, Deserialize)]
pub enum PersonEvent {
    PersonCreated { /* fields */ },
    PersonActivated { /* fields */ },
    PersonDeactivated { reason: String },
    PersonNameChanged { /* fields */ },
    PersonMerged { target_id: PersonId, reason: MergeReason },
    PersonDeceased { date_of_death: NaiveDate },
}
```

3. **Discovered Commands** (from event storming):
   - CreatePerson
   - ActivatePerson
   - DeactivatePerson
   - ChangeName
...
4. **Mapped to Rust Commands** (`src/commands/`):
```rust
#[derive(Clone, Debug)]
pub enum PersonCommand {
    CreatePerson(CreatePersonCommand),
    DeactivatePerson(DeactivatePersonCommand),
    ReactivatePerson(ReactivatePersonCommand),
    RecordDeath(RecordDeathCommand),
    MergePerson(MergePersonCommand),
}
```

5. **Discovered States** (from event storming):
   - Draft (being created)
   - Active (normal operation)
   - Suspended (temporarily inactive)
   - Archived (permanently inactive)
   - Deceased (person died)
   - Merged (duplicate merged into another)

6. **Mapped to State Machine** (`src/aggregate/person_states.rs`):
```rust
...
7. **Discovered Value Objects** (from event storming):
   - PersonName (complex, culturally-aware)
   - PersonAttribute (extensible metadata)

8. **Mapped to Rust Value Objects** (`src/value_objects/`):
```rust
...
**Result**: Complete functional domain implementation following event storming discoveries.

**Key Lesson**: Event storming directly maps to Rust enums, state machines, and pure data structures. No OOP translation needed - events ARE the code.
