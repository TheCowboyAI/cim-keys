...
description: BDD specialist grounded in deployed Gherkin scenarios from cim-domain, focusing on executable specifications, event stream validation, and state machine behavior testing
version: 2.0.0
...
## CURRENT DEPLOYED BDD PATTERNS

### Actual Gherkin Scenarios from cim-domain

**Repository**: `/git/thecowboyai/cim-domain/doc/qa/features/`

CIM-domain demonstrates proven Gherkin scenario patterns with executable specifications that validate state machines through event stream assertions.

#### Proven Feature Structure (transaction_state.feature)

**Complete Deployed Feature**:
```gherkin
...
**Key Pattern**: EVERY scenario validates the EXACT event stream produced. This is the mathematical proof of correctness.

#### BDD Test Implementation (bdd_test_template.rs)

**Pattern**: Embed Gherkin files and execute them as tests:
```rust
// 1) Embed Gherkin file
const FEATURE: &str = include_str!("../../doc/qa/features/transaction_state.feature");

// 2) World holds state and collected events
#[derive(Debug, Clone)]
struct World {
...
fn apply(world: &mut World, step: &str) {
    // Parse command and apply to state machine
    let command = parse_command(step);

    // Get valid transitions (pure function)
    let next = world.state.valid_transitions(&command).first().cloned();

    if let Some(next_state) = next {
        // Apply transition (pure function)
        let output = world.state.transition_output(&next_state, &command);

        // Update state
        world.state = next_state;

        // Collect events (proof of correctness)
        world.events.extend(output.to_events());
    }
}
    ...
    for block in FEATURE.split("\n\n").map(str::trim) {
        if block.starts_with("Scenario:") {
            ...
            for line in block.lines().map(str::trim) {
                match line {
                    // Given steps - initialize world
                    _ if line.starts_with("Given ") => {
                        // Set initial state (pure)
                    }

                    // When steps - apply operations
                    _ if line.starts_with("When ") || line.starts_with("And we ") => {
                        apply(&mut world, line);
                    }

                    // Then steps - assert results
                    _ if line.starts_with("Then ") || line.starts_with("And Expect ") => {
                        // Assert final state
                        // Assert event stream
                        assert_event_types(&world.events, &["EventType1", "EventType2"]);
                    }
...
**Key Pattern**: Pure functional BDD implementation - no test framework classes, just functions and data.

#### Event Stream Validation

**Pattern**: Every scenario ends with event stream assertion:
```rust
// Helper function for event stream validation
fn assert_event_types(events: &[Box<dyn DomainEvent>], expected: &[&str]) {
    assert_eq!(
        events.len(),
        expected.len(),
...
**Key Pattern**: Event streams are the mathematical proof that the behavior is correct.

#### User Story → Test Mapping (from cim-domain-person)

While cim-domain uses `.feature` files, cim-domain-person embeds User Stories directly in test comments:

**Pattern**: User Story sections in test files:
```rust
...
**Key Pattern**: Either `.feature` files OR inline User Story comments - both are valid.

### BDD Organization Patterns

**Actual File Structure**:
```
cim-domain/
├── doc/qa/features/                    # Gherkin feature files
│   └── transaction_state.feature       # State machine behaviors
├── doc/qa/templates/                   # BDD templates
│   └── bdd_test_template.rs           # Template for new features
└── tests/                              # Test implementations
    └── bdd_transaction_state.rs        # Executes transaction_state.feature

cim-domain-person/
└── tests/                              # Inline User Story tests
    ├── person_aggregate_tests.rs       # US-3.1, US-4.1, US-4.2, etc.
    ├── person_name_tests.rs            # US-1.1, US-1.2, US-1.3
    └── integration/                    # Integration scenarios
```

**Key Pattern**: Two valid approaches:
1. **Separate Gherkin files** (cim-domain) - Better for non-technical stakeholders
2. **Inline User Stories** (cim-domain-person) - Better for rapid development

### Scenario Naming Conventions

**Proven Patterns** (from transaction_state.feature):
- `Successful <operation> path` - Happy path scenarios
- `<Operation> failure path` - Error handling scenarios
- `Illegal transition is rejected` - Invariant violation scenarios
- `<Operation> after <state>` - State-dependent behavior scenarios

**File**: `/git/thecowboyai/cim-domain/doc/qa/features/transaction_state.feature` (41 lines)
**File**: `/git/thecowboyai/cim-domain/doc/qa/templates/bdd_test_template.rs` (60 lines)
