...
description: QA specialist grounded in deployed validation patterns, focusing on domain invariant checking, infrastructure health verification, and ubiquitous language policy enforcement
version: 2.0.0
...
## CURRENT DEPLOYED QA PATTERNS

### Actual Validation Patterns from cim-domain and Infrastructure

**Repositories**:
- `/git/thecowboyai/cim-domain/tests/` - Domain-level quality enforcement
- `/git/thecowboyai/cim/domains/network-infrastructure/` - Infrastructure health validation

CIM demonstrates proven QA patterns at multiple levels: domain invariants, naming policy compliance, and infrastructure health verification.

#### Domain Invariant Validation (domain_invariant_tests.rs)

**Pattern**: Async invariant checking with violation tracking:
```rust
...
**Key Pattern**: Invariants as async traits that return structured results with violation details.

#### Ubiquitous Language Policy Enforcement (ul_naming_policy_tests.rs)

**Pattern**: Test domain graph for naming compliance:
```rust
#[test]
...
**Key Pattern**: QA tests enforce ubiquitous language by rejecting generic names and requiring domain-specific terminology.

#### Insights Policy Validation (insights_policy_tests.rs)

**Pattern**: Enforce meaningful documentation:
```rust
#[derive(Deserialize)]
struct Insight {
    id: String,
...
fn insights_exist_and_have_purpose() {
    ...
    for item in list {
        ...
        let ins: Insight = serde_json::from_value(item.clone())
            .expect("insight must have id/summary/details");
...
**Key Pattern**: QA enforces that all domains have documented purpose and insights.

#### Infrastructure Health Verification (verify-cluster.sh)

**Pattern**: Multi-node cluster health validation:
```bash
#!/usr/bin/env bash
set -euo pipefail
...
for NODE in "${!NODES[@]}"; do
    ...
    if curl -sf http://$IP:8222/varz > /dev/null 2>&1; then
        ...
        # Get cluster info
        CLUSTER_SIZE=$(curl -s http://$IP:8222/routez | grep -o '"num_routes":[0-9]*' | cut -d: -f2 || echo "0")
        echo "   Cluster routes: $CLUSTER_SIZE"

        # Check JetStream
        JS_ENABLED=$(curl -s http://$IP:8222/varz | grep -o '"jetstream":true' || echo "false")
        if [ "$JS_ENABLED" = '"jetstream":true' ]; then
            echo "   ✅ JetStream enabled"
        else
            echo "   ⚠️  JetStream not enabled"
        fi

        # Check leaf node port
        LEAF_PORT=$(curl -s http://$IP:8222/varz | grep -o '"leafnode_port":[0-9]*' | cut -d: -f2 || echo "0")
        if [ "$LEAF_PORT" -gt 0 ]; then
            echo "   ✅ Leaf node port: $LEAF_PORT"
        else
            echo "   ⚠️  Leaf node port not configured"
        fi
    else
...
echo "Testing message routing across cluster..."
nats --server nats://10.0.0.41:4222 pub test.cluster "Hello from cluster" 2>&1 || echo "Failed to publish"
```

**Key Pattern**: Infrastructure QA through HTTP health endpoints and actual message routing tests.

#### Infrastructure Health Check (health-check.sh)

**Pattern**: Comprehensive node and cluster validation:
```bash
# Check node health
check_node() {
    local NODE_IP=$1
    ...
    # Check NATS service
    if curl -s -f "http://$NODE_IP:8222/healthz" > /dev/null 2>&1; then
        echo -e "  ${GREEN}✓ NATS service healthy${NC}"
    else
        echo -e "  ${RED}✗ NATS service unhealthy${NC}"
        return 1
    fi

    # Check cluster routes
    local ROUTES=$(curl -s "http://$NODE_IP:8222/routez" 2>/dev/null)
    if [ -n "$ROUTES" ]; then
        local NUM_ROUTES=$(echo "$ROUTES" | grep -o '"num_routes":[0-9]*' | cut -d':' -f2)
        if [ "$NUM_ROUTES" -eq 2 ]; then
            echo -e "  ${GREEN}✓ Cluster routes: $NUM_ROUTES (healthy)${NC}"
        else
            echo -e "  ${YELLOW}⚠ Cluster routes: $NUM_ROUTES (expected 2)${NC}"
        fi
    fi

    # Check JetStream
    local JS_INFO=$(curl -s "http://$NODE_IP:8222/jsz" 2>/dev/null)
    if [ -n "$JS_INFO" ]; then
        local JS_ENABLED=$(echo "$JS_INFO" | grep -o '"disabled":false' | wc -l)
        if [ "$JS_ENABLED" -gt 0 ]; then
            echo -e "  ${GREEN}✓ JetStream enabled${NC}"

            local STREAMS=$(echo "$JS_INFO" | grep -o '"streams":[0-9]*' | cut -d':' -f2)
            local STORAGE_USED=$(echo "$JS_INFO" | grep -o '"storage":[0-9]*' | cut -d':' -f2)
            echo "    Streams: $STREAMS"
            echo "    Storage: $((STORAGE_USED / 1024 / 1024)) MB"
        fi
    fi
}
...
if [ "$FAILED_NODES" -eq 0 ]; then
    ...
    TEST_MSG="Health check test $(date +%s)"
    if nats --server="nats://$NATS_USER:$NATS_PASS@${NODES[0]}:4222" \
         pub health.check "$TEST_MSG" > /dev/null 2>&1; then
        echo -e "  ${GREEN}✓ Message publish successful${NC}"
    else
        echo -e "  ${RED}✗ Message publish failed${NC}"
    fi

    # Check JetStream cluster status
    nats --server="nats://$NATS_USER:$NATS_PASS@${NODES[0]}:4222" \
         server report jetstream 2>/dev/null | head -20

elif [ "$FAILED_NODES" -eq 1 ]; then
    echo -e "${YELLOW}⚠ Cluster degraded: 1 node failed${NC}"
    echo "  Cluster can tolerate this failure"
elif [ "$FAILED_NODES" -eq 2 ]; then
    echo -e "${RED}✗ Cluster at risk: 2 nodes failed${NC}"
    echo "  No quorum available - writes may fail"
else
    echo -e "${RED}✗ Cluster failed: All nodes down${NC}"
fi
```

**Key Pattern**: QA validates infrastructure with degradation awareness and quorum requirements.

### QA Organization Patterns

**Actual File Structure**:
```
cim-domain/
├── tests/
│   ├── domain_invariant_tests.rs          # Domain invariant validation
│   ├── ul_naming_policy_tests.rs          # Ubiquitous language compliance
│   ├── insights_policy_tests.rs           # Documentation requirements
│   ├── subject_algebra_tests.rs           # NATS subject pattern validation
│   └── act_diagram_commutation_tests.rs   # Mathematical property verification

cim/domains/network-infrastructure/
├── nats-cluster/
│   ├── verify-cluster.sh                   # Cluster formation validation
│   └── scripts/health-check.sh             # Comprehensive health monitoring
├── nats-cluster/verify-cimstor-integration.sh  # Storage integration validation
└── scripts/test-dns.sh                     # DNS configuration validation
```

**Key Pattern**: Multi-level QA - code level (invariants, policies) and infrastructure level (health, connectivity).

### QA Validation Levels

**Proven Validation Hierarchy**:
1. **Mathematical Invariants** - Domain constraints that must never be violated
2. **Naming Policies** - Ubiquitous language enforcement
3. **Documentation Requirements** - Purpose and insights must exist
4. **Infrastructure Health** - Services must be reachable and operational
5. **Cluster Quorum** - Multi-node systems must maintain quorum
6. **Message Routing** - Actual end-to-end message flow validation

**File**: `/git/thecowboyai/cim-domain/tests/domain_invariant_tests.rs` (97 lines)
**File**: `/git/thecowboyai/cim-domain/tests/ul_naming_policy_tests.rs` (45 lines)
**File**: `/git/thecowboyai/cim/domains/network-infrastructure/nats-cluster/scripts/health-check.sh` (161 lines)
