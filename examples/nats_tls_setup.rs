...
use cim_keys::{
    pki::PkiManager, storage::FileKeyStorage, tls::TlsManager, CertificateFormat,
    CertificateManager, EcdsaCurve, KeyAlgorithm, RsaKeySize,
};
use std::fs;
...
async fn setup_ca_hierarchy() -> Result<(PkiManager, String), Box<dyn std::error::Error>> {
    ...
    let (_root_key, root_cert_id) = ca_manager
        .create_root_ca(
    ...
    let (_int_key, int_cert_id) = ca_manager
...
async fn generate_nats_server_certs(
    ...
    let (_server_key, server_cert_id) = tls_manager
    ...
    let server_cert_pem = tls_manager
        .export_certificate(&server_cert_id, CertificateFormat::Pem, false)
        .await?;
...
async fn generate_leaf_node_certs(
    ...
    let (_leaf_key, leaf_cert_id) = tls_manager
        .generate_self_signed(
    ...
    let leaf_cert_pem = tls_manager
        .export_certificate(&leaf_cert_id, CertificateFormat::Pem, false)
        .await?;
...
"#,
    ...
    info!(
        "Leaf node configuration written to: {}",
        leaf_config_path.display()
    );
