...
use cim_keys::{
    ...
    tls::TlsManager,
    yubikey::{PinPolicy, PivSlot, TouchPolicy, YubiKeyManager},
    CertificateFormat, CertificateManager, EcdsaCurve, EncryptionFormat, Encryptor, KeyAlgorithm,
    KeyExportFormat, KeyManager, KeyUsage, PkiOperations, RsaKeySize, SignatureFormat, Signer,
};
...
async fn setup_pki_infrastructure() -> Result<PkiManager, Box<dyn std::error::Error>> {
    ...
    let (operator_root_key, operator_root_cert) = pki_manager
        .create_root_ca(
    ...
    let (operator_int_key, operator_int_cert) = pki_manager
    ...
    let (domain_root_key, domain_root_cert) = pki_manager
        .create_root_ca(
    ...
    let (domain_int_key, domain_int_cert) = pki_manager
        .create_intermediate_ca(
            &domain_root_cert,
    ...
    // Add trusted roots
    pki_manager
        .add_trusted_root(operator_root_cert.clone(), operator_root_cert.metadata)
        .await?;

    pki_manager
        .add_trusted_root(domain_root_cert.clone(), domain_root_cert.metadata)
        .await?;
...
async fn setup_yubikey(
    ...
    match config.level {
        PkiLevel::Operator => {
            ...
            yubikey
                .import_certificate(PivSlot::Authentication, &auth_cert, CertificateFormat::Der)
                .await?;

            // Slot 9C: Digital Signature (touch required)
            let sign_key = yubikey
                .generate_key(
                    PivSlot::Signature,
                    KeyAlgorithm::Rsa(RsaKeySize::Rsa2048),
                    PinPolicy::Always,
...
async fn generate_nats_certificates(
    ...
    let (server_key, server_cert) = tls_manager
    ...
    let (leaf_key, leaf_cert) = tls_manager
        .generate_self_signed(
            "leaf-node.cim.local",
...
async fn setup_ssh_keys(yubikey: &YubiKeyManager) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let public_key = ssh_manager
        .export_key(&ssh_key, KeyExportFormat::OpenSsh, false)
        .await?;
...
/// Configure OATH TOTP
async fn configure_oath_totp(yubikey: &YubiKeyManager) -> Result<(), Box<dyn std::error::Error>> {
